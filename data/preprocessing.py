...
def exponential_smoothing(df, sensors, n_samples, alpha=0.4):
    ...
    df[sensors] = df.groupby('unit_nr', group_keys=False)[sensors].apply(lambda x: x.ewm(alpha=alpha).mean()).reset_index(level=0, drop=True)
...
def CMAPSS_preprocessor(train, test, y_test, sensors=['s_3', 's_4', 's_7', 's_11', 's_12'], sequence_length=30, alpha=0.1, threshold=125):
	...
	threshold: threshold for RUL values

	returns: x_train, y_train, x_val, y_val, x_test, y_test preprocessed
	'''

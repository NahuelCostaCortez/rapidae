...
class TrainingPipeline(BasePipeline):
    ...
    Parameters:
		model: The model to be trained. Default: None.
    output_dir: The directory where the trained model will be saved. Default: None.
    ...
    def __call__(
        ...
        if callbacks is None:
            ...
            if eval_data is None:
                callbacks.append(EarlyStopping(monitor='loss', patience=10, verbose=1, mode='min'))
                callbacks.append(ModelCheckpoint(filepath=self.output_dir, monitor='loss', verbose=1, save_best_only=True, mode='min', save_weights_only=True))
            else:
                callbacks.append(EarlyStopping(monitor='val_loss', patience=10, verbose=1, mode='min'))
                callbacks.append(ModelCheckpoint(filepath=self.output_dir, monitor='val_loss', verbose=1, save_best_only=True, mode='min', save_weights_only=True))
